...
// Doctor Finder Routes (Public)
Route::get('/find-doctor', [PublicDoctorController::class, 'findDoctorPage'])->name('doctors.find');
Route::get('/doctor/{id}/schedule', [PublicDoctorController::class, 'getWeeklySchedule'])->name('doctor.schedule');

// Public Appointment Routes
Route::get('/schedule-appointment', [PublicAppointmentController::class, 'showAppointmentForm'])->name('public.appointment.create');
Route::post('/doctor-available-days', [PublicAppointmentController::class, 'getDoctorAvailableDays'])->name('public.doctor.available.days');
Route::post('/doctor-available-slots', [PublicAppointmentController::class, 'getDoctorAvailableSlots'])->name('public.doctor.available.slots');
Route::post('/schedule-appointment', [PublicAppointmentController::class, 'store'])->name('public.appointment.store');
Route::get('/appointment-success', [PublicAppointmentController::class, 'success'])->name('public.appointment.success');
Route::get('/insert-test-data', [PublicAppointmentController::class, 'insertTestData'])->name('public.insert.test.data');

// Public Call Request Routes
Route::get('request-call', [App\Http\Controllers\CallSettingController::class, 'showCallForm'])->name('request-call.form');
Route::post('request-call', [App\Http\Controllers\CallSettingController::class, 'processCallRequest'])->name('request-call.process');
...
Route::middleware('sentinel.auth')->group(function () {
    ...
    // Call settings - Para admin (se verifica en el layout sidebar)
    Route::get('setting-call', [App\Http\Controllers\CallSettingController::class, 'index'])->name('setting-call.index');
    Route::get('setting-call/create', [App\Http\Controllers\CallSettingController::class, 'create'])->name('setting-call.create');
    Route::post('setting-call', [App\Http\Controllers\CallSettingController::class, 'store'])->name('setting-call.store');
    Route::get('setting-call/{id}/edit', [App\Http\Controllers\CallSettingController::class, 'edit'])->name('setting-call.edit');
    Route::put('setting-call/{id}', [App\Http\Controllers\CallSettingController::class, 'update'])->name('setting-call.update');
    Route::delete('setting-call/{id}', [App\Http\Controllers\CallSettingController::class, 'destroy'])->name('setting-call.destroy');
    ...
    Route::get('appointment/{id}/budget', 'AppointmentController@appointmentBudgetView')->name('appointment.budget.view');
    Route::get('appointment/{id}/budget/whatsapp', 'AppointmentController@appointmentBudgetWhatsApp')->name('appointment.budget.whatsapp');
    ...
    // Doctor Schedule AJAX Route
    Route::get('/doctors/{id}/schedule', 'DoctorController@getWeeklySchedule')->name('doctors.schedule');
});
